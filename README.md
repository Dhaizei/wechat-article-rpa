...
> 在 Windows 微信客户端中按公众号采集文章，提取正文、发布时间与互动数据，并幂等写入 MongoDB；同时提供管理员控制台和团队只读资讯页面。
...
- 通过微信“搜一搜”的 **账号 → 公众号** 筛选，定位公众号资料窗口；不要求提前关注公众号。
...
- 写入前校验文章窗口标题、正文标题、文章 URL 与互动栏处于同一文章，避免错配入库。

### 管理与展示能力

- 管理员控制台：手动采集、定时任务、启动前检查、实时日志、文章管理、公众号管理、导入导出。
- 团队资讯页：每日新闻、文章动态、公众号目录，适合局域网同事只读浏览。
- 日报按日期归档，支持重点摘要、分类解读、热点文章及原文链接。
...
| 类型 | 页面 | 权限 |
|---|---|---|
| 管理员控制台 | `/`、`/accounts.html`、`/articles.html` | HTTP Basic 管理员认证 |
| 团队只读资讯页 | `/briefing.html`、`/daily.html`、`/directory.html` | 可在局域网只读访问 |

团队阅读页面不会提供启动采集、停止任务、修改配置、账号编辑或导入导出能力。**不要把管理员控制台直接开放到公网。**
...
以下页面适合给其他同事查看，不包含采集控制权限：

| 页面 | 地址 | 内容 |
|---|---|---|
| 每日新闻 | `/briefing.html` | 每日早报、热点、编辑摘要、分类解读、原文入口 |
| 文章动态 | `/daily.html` | 某日文章流、热点榜、公众号动态、分类筛选 |
| 公众号目录 | `/directory.html` | 按分类浏览公众号及其最新文章 |

本机示例：

```text
http://127.0.0.1:8010/briefing.html
http://127.0.0.1:8010/daily.html
http://127.0.0.1:8010/directory.html
```

日报数据来自 MongoDB 的 `weixin.daily_reports`，由 `daily_news_send` 生成；资讯页面只读展示，不会改变日报发送状态。
...
默认服务只监听 `127.0.0.1`。若需要让局域网同事查看日报页面，在 VM 上用以下方式启动：
...
例如 VM IP 为 `192.168.1.88`，团队访问：

```text
http://192.168.1.88:8010/briefing.html
http://192.168.1.88:8010/daily.html
http://192.168.1.88:8010/directory.html
```
...
```powershell
New-NetFirewallRule `
  -DisplayName "WeChat RPA Team Dashboard 8010" `
  -Direction Inbound `
  -Action Allow `
  -Protocol TCP `
  -LocalPort 8010 `
...
1. 在微信搜一搜输入目标账号名称；
2. 确认一级分类“账号”和二级分类“公众号”均被选中；
3. 通过本地 OCR 找到同名公众号结果，再打开独立资料窗口；
4. 按时间分组读取文章卡片，向下滚动至“星期 N / 周 N / 明确年月日”等历史边界；
5. 依次打开文章，复制链接解析正文；
6. 校验浏览器标签标题、正文标题和当前 URL，再读取转发数；
7. 文章完成后关闭文章标签；账号完成后关闭资料窗口；保留搜一搜主窗口。
...
1. **文章卡片层**：时间分组、规范化标题、列表阅读数和列表点赞数均相同才跳过；任何字段缺失或变化都会继续打开，宁可多读不误漏。
2. **文章链接层**：打开后若 URL 与本次已成功文章完全相同，立即关闭，不再重复解析正文和互动数。

标题 OCR 截断或相似标题不会触发跳过；失败文章允许后续重试。去重集合不会跨任务保留，因此后续运行仍会刷新互动数据。
...
先确认微信已登录并且“搜一搜”窗口存在且没有被最小化。控制台的“重新检测 / 自动恢复”会尝试拉起窗口；如果仍失败，手工在微信顶部搜索框输入“搜一搜”，在候选项中点击“搜一搜”后重新检测。
...
这是安全校验的一部分。程序会在互动数识别前后分别复制 URL，只有两次 URL 相同才认为互动数与正文属于同一篇文章。失败文章会在有限次数内重试；成功 URL 会在当前任务中去重。
