...
- 兼容微信标准和自定义标签栏；优先直接定位新建标签，无法确认时回退到保守的完整遍历。
- 支持在仅转发数模式下读取经校验的网络响应缓存；未命中时仍使用既有 UIA、OCR 和视觉模型回退。
- 记录运行入口、代码目录与导航回退原因，便于排查远程部署未更新或微信界面变化。
...
- 增加浏览器标签归属管理，避免在复用标签或异常退出后误关闭用户原有页面。
- 优化公众号和文章直接导航、标题定位及图片消息解析，并补充相应回归测试。
- 新增 [标签归属与清理说明](docs/browser-tab-lifecycle.md)、[运行诊断说明](docs/runtime-diagnostics.md) 和 [网络转发数采集说明](docs/wechat-network-capture.md)。
...
该记录表示微信窗口识别、搜一搜、公众号资料页和文章采集流程已在上述组合中运行验证，并不代表所有 Windows 7 裸机都能直接完成依赖安装。项目的一键安装脚本默认使用 64 位 Python 3.10；新部署仍推荐 Windows 11 64 位。若使用 Windows 7，请优先复用已经验证过的系统镜像，并在正式采集前通过控制台完成启动前检查和单账号试采集。
